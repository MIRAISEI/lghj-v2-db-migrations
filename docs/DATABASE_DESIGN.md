...
| `case_vehicles` | Per-vehicle bidding/pricing/deposit/LC/balance tracking | `vehicle_status` ENUM(`PENDING`,`IN_BIDDING`,`BID_WIN`,`BID_LOSS`,`IGNORED`,`IN_INVENTORY`†); `purchase_status` ENUM(`PENDING`,`PURCHASED`,`CANCELLED`) DEFAULT `PENDING` (V72, won-vehicle purchase tracking — see gotcha #16); `cif_breakdown_json`/`metadata_json` JSON | `case_id`→cases.id (CASCADE) |
...
| Table | Purpose | Key columns | FKs |
|---|---|---|---|
| `inventory_vehicles` | Vehicle stock record (from won bids or manual entry) **and** public-portal listings — told apart by `stock_type` (V76, see gotcha #17) | `id` DEFAULT(UUID()); `stock_id` NOT NULL + UNIQUE (V47, see gotcha #9); `stock_type` NOT NULL default `'Real Stock'`, idx (`'Public Stock'` = portal listing); `source_vehicle_id` UNIQUE (V76, the real vehicle a public listing was copied from); `last_synced_at`; `status` default `ACTIVE`; `case_vehicle_id`/`source_case_id` | `case_vehicle_id`/`source_case_id` — indexed only, no FK (see gotcha #12); `source_vehicle_id`→inventory_vehicles.id (SET NULL, V76) |
...
16. **`case_vehicles.winning_bid_price` was a dead column until V72.** Present since the V6 baseline but never written by any application code. V72 adds `case_vehicles.purchase_status` ENUM(`PENDING`,`PURCHASED`,`CANCELLED`) for per-vehicle purchase tracking of won vehicles, and admin-api now writes `winning_bid_price` as the staff-entered "Purchase Value" alongside it (from the CRM bid-win dialog and Purchasing tab). The matching "Purchase Receipt" is a `case_documents` row with `document_type = 'PURCHASE_RECEIPT'` — no schema change for that part.
